    ...
    class ItemSlotMonitor(val slotId: () -> Int) {
        ...
        private fun shouldHandleItem(): Boolean {
            ...
            if (currentItem.isEmpty()) {
                return !(AutoRefillSettings.DISABLE_FOR_LOYALTY_ITEMS.value &&
                        storedItem.itemType.vanillaStack.enchantments.getLevel(Enchantments.LOYALTY) != 0)
            }
            ...
            if (itemType.isDamageable) {
                if (AutoRefillSettings.REFILL_BEFORE_TOOL_BREAK.booleanValue && !tempDisabledForDamageable) {
                    if (!(AutoRefillSettings.ALLOW_BREAK_FOR_NON_ENCHANTED.value
                                && itemType.vanillaStack.get(DataComponentTypes.ENCHANTMENTS)!!.isEmpty()
            ...
            if (AutoRefillSettings.VISUAL_REPLACE_SUCCESS_NOTIFICATION.value) {
                ...
                when (AutoRefillSettings.TYPE_VISUAL_REPLACE_SUCCESS_NOTIFICATION.value) {
                    ToolReplaceVisualNotification.SUBTITLE -> {
                        showSubTitle(TextSerializer.fromJson(message(false), WrapperLookup.of(Stream.empty())));
                    }
                    ToolReplaceVisualNotification.HOTBAR   -> Vanilla.inGameHud().setOverlayMessage(TextSerializer.fromJson(message(false), WrapperLookup.of(Stream.empty())),
                                                                                                    false)
                    ToolReplaceVisualNotification.CHAT     -> VanillaUtil.chat(TextSerializer.fromJson(message(true), WrapperLookup.of(Stream.empty()))!!)
                }
            }
            ...
            if ((AutoRefillSettings.VISUAL_DURABILITY_NOTIFICATION.value
                ...
                if (AutoRefillSettings.VISUAL_DURABILITY_NOTIFICATION.value) {
                    ...
                    when (AutoRefillSettings.TYPE_VISUAL_DURABILITY_NOTIFICATION.value) {
                        ...
                        ToolReplaceVisualNotification.HOTBAR -> Vanilla.inGameHud().setOverlayMessage(TextSerializer.fromJson(message(false), WrapperLookup.of(Stream.empty())), false)
                        ToolReplaceVisualNotification.CHAT -> VanillaUtil.chat(TextSerializer.fromJson(message(true), WrapperLookup.of(Stream.empty()))!!)
                    }
